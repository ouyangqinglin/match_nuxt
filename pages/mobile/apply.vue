<template>
  <div class="mobile-apply" :style="{color: themeColor}">
      ...
      <div class="top-box">
        <template v-for="i of companyFields">
          ...
          <common-flex class="form" align="center" v-else-if="i.type === 'number'">
            <div class="star" v-if="+i.required === 1" />
            <el-input inputmode="decimal" pattern="[0-9]*" @blur="inputVerify(i, i.value)" type="number" v-model.number="i.value" :placeholder="i.placeholder"></el-input>
            <common-flex align="center" class="form-errMsg">{{ i.errMsg }}</common-flex>
          </common-flex>
    ...
    <div class="bottom" style="margin-top: .45rem" v-for="(product, j) of productFields" :key="j">
      <div class="bottom-header">参赛产品信息</div>
      ...
      <div class="posi bottom-box">
        ...
        <template v-for="i of product">
          <common-flex class="form"  v-if="i.type === 'text'" align="center">
            <div class="star" v-if="+i.required === 1" />
            <el-input @blur="inputProVerify(i, j, i.value)" :disabled="!!+(i.disabled)" v-model.trim="i.value" :placeholder="i.placeholder"></el-input>
            <common-flex align="center" class="form-errMsg">{{ i.errMsg }}</common-flex>
          </common-flex>
          ...
          <common-flex class="form" v-else-if="i.type === 'number'">
            <div class="star" v-if="+i.required === 1" />
            <el-input inputmode="decimal" pattern="[0-9]*" :disabled="!!+(i.disabled)" @blur="inputProVerify(i, j, i.value)" type="number" v-model.number="i.value" :placeholder="i.placeholder"></el-input>
    ...
    <common-flex class="btn-container" direction="column" align="center">
      ...
      <common-flex class="agree" justify="center" align="center">
        ...
        <div>我已阅读</div>
        <div class="book" v-if="commitment_letter" @click="agreeDetail(1)">《参赛承诺书》</div>
...
.mobile-apply {
  ...
  .top, .bottom {
    ...
    &-header {
      ...
      line-height: .7rem;
      text-align: center;
      @include bg(url('~@img/mobile/apply-header-bg.svg'));
    }
    .title {
      margin: 0 auto;
      width: 6.7rem;
